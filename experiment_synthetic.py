...
parser.add_argument(
    ...
    choices=["led", "oracle-ibm", "oracle-wiener", "cacgmm", "sepformer"]
)
parser.add_argument(
    "--speech-cov", 
    type=str, 
...
def process_experiment(SIR, mic, bf, mask, speech_cov, args):
    ...
    X_mix = analysis(mix) # [T x F x M]
    nTime, nFreq, nMic = X_mix.shape
    ...
    assert S_ref.shape == N_ref.shape == X_mix.shape[:2]
    ...
    if mask == 'oracle-ibm':
        ...
        X_noise = X_mix * oracle_mask_noise
        ...
        from pb_bss.distribution import CACGMMTrainer, GCACGMMTrainer
        ...
        plt.subplot(1,2,1)
        plt.imshow(affiliation_pa[0,:,:], aspect='auto')
        plt.title('Speech affiliation 1')
        plt.colorbar()
        plt.subplot(1,2,2)
        plt.title('Noise affiliation 2') 
        plt.imshow(affiliation_pa[1,:,:], aspect='auto')
        plt.colorbar()
        plt.tight_layout()
        ...
        X_speech = masks[0,...,None] * X_mix if speech_cov == "masked" else X_mix
        X_noise = masks[1,...,None] * X_mix
        ...
        est_sources = model.separate_file(path=file_pattern.format(mic_choices[mic], "mix", SIR))
    
    elif mask == "neuralfcasa":
        pass
    ...
    assert Rn.shape == (nFreq, nMic, nMic)
    ...
    if bf == 'maxsinr':
        ...
        for i in range(n_channels):
            z.append(compute_gain(w, X_speech, X_speech[:, :, i], clip_up=args.clip_gain))
        ...
        w_r = []
        for i in range(nMic):
            w = mvdr_weights(room.sources[0].position[:,None], room.mic_array.R, room.c, freqs[1:], Rx[1:], i, 1e-6, True)
            w = np.concatenate([np.zeros((1, w.shape[1])), w], axis=0)
            w_r.append(w)
        w_r = np.stack(w_r, axis=-1)
    ...
    snr_per_chan = np.einsum('fir,fij,fjr->r', w_r.conj(), Rs, w_r) / np.einsum('fir,fij,fjr->r', w_r.conj(), Rn, w_r)
    ...
    # Signal alignment step
    ref = np.vstack([speech_ref[:, ref_mic_idx], noise_ref[:, ref_mic_idx]])
    # Not sure why the delay is sometimes negative here... Need to check more
    delay = np.abs(int(pra.tdoa(out, speech_ref[:, ref_mic_idx].astype(float), phat=True)))
    if delay > 0:
        out_trunc = out[delay : delay + ref.shape[1]]
        noise_eval = mix[: ref.shape[1], ref_mic_idx] - out_trunc
    else:
    ...
    bf_contribution_time = []
    bf_incremental_contribution_time = []
    
    for j in range(len(room.sources)):
        
        name = source_echoes[j]['name']
        atfs_fji = source_echoes[j]['atfs_fji']
        incrememntal_atf = np.cumsum(atfs_fji, axis=1)
        
        contribution = np.einsum('fji,tf->tfji', atfs_fji, sources_freq_signal[j])
        incremental_contribution = np.einsum('fji,tf->tfji', incrememntal_atf, sources_freq_signal[j])
        ...
        incremental_contribution_time.append(_stacked_synthesis(incremental_contribution)) # [T x nDoas x nChan]
    ...
    for k in range(N_IMAGES_PERFORMANCE):
        ...
        # Signal alignment step
        ref = np.vstack([contribution_time[0][:, j, ref_mic_idx], contribution_time[1][:, j, ref_mic_idx]])
        # Not sure why the delay is sometimes negative here... Need to check more
        delay = np.abs(int(pra.tdoa(out, contribution_time[0][:, j, ref_mic_idx].astype(float), phat=True)))
        if delay > 0:
            out_trunc = out[delay : delay + ref.shape[1]]
            L = min(ref.shape[1], out_trunc.shape[0], mix.shape[0])
            noise_eval = mix[: L, ref_mic_idx] - out_trunc[: L]
            out_trunc = out_trunc[: L]
        else:
            out_trunc = np.concatenate((np.zeros(-delay), out[: ref.shape[1] + delay]))
            L = min(ref.shape[1], out_trunc.shape[0], mix.shape[0])
            noise_eval = mix[: L, ref_mic_idx] - out_trunc[: L]
        ...
        if True:
            ...
            axarr[0, 0].plot(ref[0])
            axarr[0, 0].plot(sig_eval[0])
            axarr[0, 1].plot(ref[1])
    ...
    for k in range(N_IMAGES_PERFORMANCE):
        ...
        # Signal alignment step
        ref = np.vstack([incremental_contribution_time[0][:, j, ref_mic_idx], incremental_contribution_time[1][:, j, ref_mic_idx]])
        # Not sure why the delay is sometimes negative here... Need to check more
        delay = np.abs(int(pra.tdoa(out, incremental_contribution_time[0][:, j, ref_mic_idx].astype(float), phat=True)))
        if delay > 0:
            out_trunc = out[delay : delay + ref.shape[1]]
            L = min(ref.shape[1], out_trunc.shape[0], mix.shape[0])
            noise_eval = mix[: L, ref_mic_idx] - out_trunc[: L]
    ...
    #     rel_incremental_svects = np.cumsum(svects, axis=1) / svects[...,ref_mic_idx][:,:,None]  # Freq x nDoas x nChan
                
    #     rel_svects = compute_steering_vector(src_pos, mic_pos, room.c, freqs, ref_mic_idx=ref_mic_idx) # Freq x nDoas x nChan
    #     assert svects.shape == rel_svects.shape == rel_incremental_svects.shape
                
    #     correlation_per_svect = np.real(np.einsum('fm,fsm->fs', bf_weights.conj()[1:], rel_svects[1:]))    
    #     correlation_rel_inc = np.real(np.einsum('fm,fsm->fs', bf_weights.conj()[1:], rel_incremental_svects[1:]))
        
    #     ri_sv_norm = np.linalg.norm(rel_incremental_svects[1:], axis=-1).real
    #     correlation_rel_inc /=  ri_sv_norm
        
    #     if True:
    #         fig, axarr = plt.subplots(2, 1, figsize=(12, 6))
            
    #         im = axarr[0].imshow(correlation_per_svect, aspect='auto')
    #         plt.colorbar(im, ax=axarr[0])
    #         axarr[0].set_title(f'Correlation with {name} images')
    #         axarr[0].set_ylabel('Frequency')
    #         axarr[0].set_xlabel('Image index')
            
    #         im = axarr[1].imshow(np.abs(correlation_rel_inc), aspect='auto')
    #         plt.colorbar(im, ax=axarr[1])
    #         axarr[1].set_title(f'Relative (incremental steering vectors)')
